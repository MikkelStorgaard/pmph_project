...
int Colonies3D::Run_LoopDistributed_GPU(numtype T_end)
{
	...
	{
		fprintf(stderr, "Failed to allocate arr_nC on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_Occ on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_IsActive on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_partialSum on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate d_rng_state on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy rng_state to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_B on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_B_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_P on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_P_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I0 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I0_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I1 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I1_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I2 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I2_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I3 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I3_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I4 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I4_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I5 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I5_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I6 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I6_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I7 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I7_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I8 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I8_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I9 on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_I9_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_M on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_p to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_nutrient on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_nutrient_new on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_GrowthModifier to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_0 to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_u to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_d to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_l to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_r to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_f to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate arr_n_b to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate Warn_g on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate d_Warn_fastGrowth on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate Warn_r on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate Warn_delta on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate Warn_density on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	for (int n = 0; n < nSamplings; n++)
	{
		...
		for (int t = 0; t < nStepsPerSample; t++)
		{
			...
			if (GPU_NC)
			{
				...
				if (err != cudaSuccess && errC > 0)
				{
					fprintf(stderr, "Failure in FirstKernel! error = %s\n", cudaGetErrorString(err));
					errC--;
				}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SetIsActive! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (GPU_MAXOCCUPANCY)
			{
				...
				if (err != cudaSuccess && errC > 0)
				{
					fprintf(stderr, "Failure in SecondKernel! error = %s\n", cudaGetErrorString(err));
					errC--;
				}
				...
				if (!OPTIMIZED_MAXOCCUPANCY)
				{
					...
					if (err != cudaSuccess && errC > 0)
					{
						fprintf(stderr, "Failure in SequentialReduceMax! error = %s\n", cudaGetErrorString(err));
						errC--;
					}
				}
				...
				if (!OPTIMIZED_MAXOCCUPANCY)
				{
					...
					if (err != cudaSuccess && errC > 0)
					{
						fprintf(stderr, "Failed to copy arr_partialSum to the host! error = %s\n", cudaGetErrorString(err));
						errC--;
					}
				}
			}
			...
			if (GPU_BIRTH)
			{
				...
				if (err != cudaSuccess && errC > 0)
				{
					fprintf(stderr, "Failure in ComputeBirthEvents! error = %s\n", cudaGetErrorString(err));
					errC--;
				}
			...
			if (GPU_BIRTH)
			{ // We still need to compute the skip array
				for (int i = 0; i < nGridXY; i++)
				{
					for (int j = 0; j < nGridXY; j++)
					{
			...
			if (GPU_INFECTIONS)
			{
				...
				if (err != cudaSuccess && errC > 0)
				{
					fprintf(stderr, "Failure in BurstingEvents or NonBurstingEvents! error = %s\n", cudaGetErrorString(err));
					errC--;
				}
				...
				for (int i = 0; i < nGridXY; i++)
				{
					...
					for (int j = 0; j < nGridXY; j++)
					{
						...
						for (int k = 0; k < nGridZ; k++)
						{
							...
							// Skip empty sites
							if (skipArray[i * nGridXY * nGridZ + j * nGridZ + k])
								continue;
					...
					for (int j = 0; j < nGridXY; j++)
					{
						...
						for (int k = 0; k < nGridZ; k++)
						{
							...
							if (nGridXY > 1)
							{
								...
								{ // Reflective boundaries in Z direction
								...
								if (r > 0.0)
								{
									...
									arr_I1_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I1_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I1_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I1_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I1_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I1_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I2_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I2_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I2_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I2_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I2_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I2_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I3_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I3_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I3_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I3_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I3_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I3_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I4_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I4_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I4_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I4_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I4_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I4_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I5_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I5_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I5_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I5_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I5_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I5_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I6_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I6_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I6_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I6_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I6_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I6_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I7_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I7_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I7_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I7_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I7_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I7_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I8_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I8_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I8_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I8_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I8_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I8_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
									...
									arr_I9_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
									arr_I9_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
									arr_I9_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
									arr_I9_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
									arr_I9_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
									arr_I9_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
								}
								...
								arr_P_new[ip * nGridXY * nGridZ + j * nGridZ + k] += n_u;
								arr_P_new[im * nGridXY * nGridZ + j * nGridZ + k] += n_d;
								arr_P_new[i * nGridXY * nGridZ + jp * nGridZ + k] += n_r;
								arr_P_new[i * nGridXY * nGridZ + jm * nGridZ + k] += n_l;
								arr_P_new[i * nGridXY * nGridZ + j * nGridZ + kp] += n_f;
								arr_P_new[i * nGridXY * nGridZ + j * nGridZ + km] += n_b;
		...
		if (err != cudaSuccess && errC > 0)
		{
			fprintf(stderr, "Failure in PartialSum (B)! error = %s\n", cudaGetErrorString(err));
			errC--;
		}
		...
		if (err != cudaSuccess && errC > 0)
		{
			fprintf(stderr, "Failure in SequentialReduceSum (B)! error = %s\n", cudaGetErrorString(err));
			errC--;
		}
		...
		if (!exportAll)
		{
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I0)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I0)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I1)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I1)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I2)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I2)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I3)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I3)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I4)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I4)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I5)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I5)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I6)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I6)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I7)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I7)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I8)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I8)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (I9)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (I9)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (P)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (P)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum (nutrient)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum (nutrient)! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in PartialSum! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
			...
			if (err != cudaSuccess && errC > 0)
			{
				fprintf(stderr, "Failure in SequentialReduceSum! error = %s\n", cudaGetErrorString(err));
				errC--;
			}
...
int Colonies3D::Run_LoopDistributed_CPU_cuRand(numtype T_end)
{
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to allocate d_N on the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
				...
				for (int j = 0; j < nGridXY; j++)
				{
					...
					for (int k = 0; k < nGridZ; k++)
					{
						...
						if (GPU_BIRTH)
						{
							...
							if (err != cudaSuccess && errC > 0)
							{
								fprintf(stderr, "Failure in cuRandKernel! error = %s\n", cudaGetErrorString(err));
								errC--;
							}
				...
				for (int j = 0; j < nGridXY; j++)
				{
					...
					for (int k = 0; k < nGridZ; k++)
					{
						...
						if ((arr_Occ[i * nGridXY * nGridZ + j * nGridZ + k] >= 1) and (arr_P[i * nGridXY * nGridZ + j * nGridZ + k] >= 1))
						{
							// Compute the number of hits
							if (eta * s * dT >= 1)
							{ // In the diffusion limited case every phage hits a target
								N = arr_P[i * nGridXY * nGridZ + j * nGridZ + k];
							}
							else
							{
...
void Colonies3D::CopyAllToHost()
{
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_r to the host! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_delta to the host! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_g to the host! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_fastGrowth to the host! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
}
...
void Colonies3D::CopyAllToDevice()
{
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_r to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_delta to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_g to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_fastGrowth to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
	...
	if (err != cudaSuccess && errC > 0)
	{
		fprintf(stderr, "Failed to copy Warn_density to the device! error = %s\n", cudaGetErrorString(err));
		errC--;
	}
};
