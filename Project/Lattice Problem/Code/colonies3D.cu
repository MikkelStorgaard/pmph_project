...
int Colonies3D::Run_LoopDistributed_CPU(double T_end) {
    ...
    for (int n = 0; n < nSamplings; n++) {
      ...
      int blockDim = 8;
      dim3 blockDim3(blockDim, blockDim, blockDim); // going with this since 8**3 is 512, a Good block size

      int xBlocks = (nGridXY + blockDim -1) / blockDim;
      int yBlocks = (nGridXY + blockDim -1) / blockDim;
      int zBlocks = (nGridZ + blockDim -1) / blockDim;
      dim3 gridSize(xBlocks, yBlocks, zBlocks);


      double*** d_arr_Occ, d_arr_nC;
      double* d_maxOccupancy;

      cudaMalloc((void**)&d_arr_nC , totalMemSize);
      cudaMalloc((void**)&d_arr_Occ, totalMemSize);
      cudaMalloc((void**)&d_maxOccupancy, sizeof(double));
