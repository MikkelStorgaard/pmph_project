...
Simulation::Simulation(int N_0) {
    ...
    L                   = 50;       // [µm]     Length of boundary condition box
    ...
    h_colony            = 125;      // [µm]    Distance of the cell colony to the hard agar
    ...
    forcedInfections    = 0;        // "Boolean" to force an infected surface of a given number
...
Simulation::Simulation(const Simulation& other) {
    ...
    h_colony            = other.h_colony;                   // [µm]    Distance of the cell colony to the hard agar
    ...
    forcedInfections    = other.forcedInfections;           // "Boolean" to force an infected surface of a given number
...
int Simulation::Run(double T) {
    ...
    if ((Time < (int)round(T_i / dT)) or (nSamplings == 0)) {
        ...
        nTimeStepsPerSample = (int)round(1 / (nSamp * dT));
    }
    ...
    for (int n = 0; n < nSamplings; n++) {
        ...
        if (fabs(nSamplings * nTimeStepsPerSample * dT - T) > dT / 2) {
    ...
    if (debug > 0) {
        f_benchMarking.open(path + "/benchmark.txt", fstream::out);
        f_benchMarking << "\tBenchmarking results:" << endl;
        f_benchMarking << "\t"       << setw(3) << difftime(toc, tic) << " s of total time" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[0]  << " s spent on computing movement" << endl;
        f_benchMarking << "\t\t"     << setw(3) << benchMark[7]  << " s spent on computing cell bursting" << endl;
        f_benchMarking << "\t\t"     << setw(3) << benchMark[8]  << " s spent on computing cell movement" << endl;
        f_benchMarking << "\t\t"     << setw(3) << benchMark[9]  << " s spent on computing phage movement" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[12] << " s spent on phage-cell overlap and diffusion" << endl;
        f_benchMarking << "\t\t\t\t" << setw(3) << benchMark[18] << " s spent on phage-cell overlap" << endl;
        f_benchMarking << "\t\t\t\t" << setw(3) << benchMark[19] << " s spent on phage diffusion" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[13] << " s spent on phage-boundary reflections" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[1]  << " s spent on executing movement" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[2]  << " s spent on cell growth" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[3]  << " s spent on nutrient" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[4]  << " s spent on infections" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[5]  << " s spent on CN, B and P grids" << endl;
        f_benchMarking << "\t\t"     << setw(3) << benchMark[10] << " s spent on armadillo data structures" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[14] << " s spent on clearing arrays" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[15] << " s spent on updating arrays" << endl;
        f_benchMarking << "\t\t"     << setw(3) << benchMark[11] << " s spent on std vector data structures" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[16] << " s spent on clearing arrays" << endl;
        f_benchMarking << "\t\t\t"   << setw(3) << benchMark[17] << " s spent on updating arrays" << endl;
        f_benchMarking << "\t"       << setw(3) << benchMark[6]  << " s spent on filesaving" << endl;
        f_benchMarking.close();
...
void Simulation::Initialize() {
    ...
    if (debug > 1) {
        cout << endl;
        cout << "\tInitlizing the simulation.." << endl;
    }

    // Average nutrient per unit cell
    nutrient = n_0*pow(dGrid*R,3);
    ...
    // Resize the clever neighbour grid
    CN.resize(2*res+1);
    ...
    // Initilize the cells at (0,0,0) + random offset, w        ith radius r = 0.677+random offset, and in the 0 state
...
void Simulation::CellMovement(int I, double *B) {
    ...
    for (int i = 0; i < NN.size(); i++) {
        ...
        if ( d < cells[n][3]+cells[I][3] ) {
            ...
            // See notes p. 12
            double F = - PotentialGradient(d,cells[n][3],cells[I][3]) * dT;
...
void Simulation::GrowCell(int I) {
    ...
    double g = 0.0;
    if (K > 0.0) g = g_max * nutrient / (nutrient + K);
    else g = g_max;

    // Assuming exponential growth
    cells[I][3] += dT * (g / 3.0) * cells[I][3] ;
...
bool Simulation::GrowInfection(int I) {
    ...
    if ((cells[I][4] > 0) and (rand(rng) <= r * g * dT)) {
        cells[I][4]++;
    }
...
void Simulation::ApplyBoundaryConditions(int m) {
    ...
    if (phages[m][0] >  L / 2) {
        ...
        phages[m][0] -= 2*(phages[m][0] - L / 2);
    }
    if (phages[m][0] < - L / 2) {
        if (absorbingBoundary) {phages.erase(phages.begin() + m); return;}
        phages[m][0] += 2*(- L / 2 - phages[m][0]);
    }

    // Check Y-direction
    if (phages[m][1] >  L / 2) {
        if (absorbingBoundary) {phages.erase(phages.begin() + m); return;}
        phages[m][1] -= 2*(phages[m][1] - L / 2);
    }
    if (phages[m][1] < - L / 2) {
        if (absorbingBoundary) {phages.erase(phages.begin() + m); return;}
        phages[m][1] += 2*(- L / 2 - phages[m][1]);
    }
    ...
    if (reflectingBoundary or absorbingBoundary) {
        ...
        if (phages[m][2] >  L / 2) {
            if (absorbingBoundary) {phages.erase(phages.begin() + m); return;}
            phages[m][2] -= 2*(phages[m][2] - L / 2);
        }
        if (phages[m][2] < - L / 2) {
            if (absorbingBoundary) {phages.erase(phages.begin() + m); return;}
            phages[m][2] += 2*(- L / 2 - phages[m][2]);
        }

    } else if (experimentBoundary) {

        if (phages[m][2] > h_agar - h_colony) {   // Reflecting top boundary
            phages[m][2] -= 2 * (phages[m][2] - h_agar + h_colony);
        } else if (phages[m][2] < - h_colony) { // Absorbing hard agar
            phages.erase(phages.begin() + m);
        }
    }
}
...
void Simulation::SpawnPhages() {
    ...
    if (planarPhageInvasion) {
        ...
        p += FirstPassageTimeDistribution() * dT * (int)(P_0 * pow(L,2));

        // Spawn phages
        while (p > 1) {

            // Random entry along XY plane (Uniformly)
            double x = (rand(rng)-0.5)*L;
            double y = (rand(rng)-0.5)*L;
    ...
    if (forcedInfections > 0) {
        ...
        for (int i = 0; i < forcedInfections; i++) {
            r_max = 0;
            I_max = -1;
            for (int n = 0; n < N; n++) {
...
double Simulation::FirstPassageTimeDistribution() {
    ...
    double d = 0.0;
    d = h_agar - h_colony - ( res + 0.5 ) * dGrid;
...
void Simulation::WriteLog(double T) {
    if ((not f_log.is_open()) and (not exit)) {
        ...
        f_log << "L = "    << L    << endl;        // Length of boundary condition box
...
void Simulation::SaveState() {
    ...
    f_out << "L="   << fixed        << L                    << endl;    // Length of boundary condition box
    ...
    while( getline(is_file, line) ) {
        ...
        if( getline(is_line, key, '=') ) {
            ...
            if( getline(is_line, value) ) {
                ...
                else if (key.compare("L")                  == 0) { istringstream(value) >> L; }
...
Simulation& Simulation::operator=(const Simulation& rhs){
    ...
    L       = rhs.L;                              // [µm]     Length of boundary condition box
